...
\label{def:sld}
The relative Split-Landscape Discrepancy of order $p$ between dataset $D$ and
surrogate $D'$ is
%
\begin{equation}
  \label{eq:sld}
  \mathrm{SLD}_p(D, D') \;=\;
  \frac{\bigl\lVert \Gamma_D - \Gamma_{D'} \bigr\rVert_p}
...
\begin{equation}
  \label{eq:prop1}
  \mathrm{SLD}_\infty(D, D') < \Delta_v / 2,
\end{equation}
%
...
measured margin-qualifying cells reported in \Cref{sec:results-ordering},
root-level split agreement equals $1.0$. The dose-response experiment
(\cref{sec:results-dose}) provides the empirical validation.

\paragraph{Coverage guarantee.}
The split-coverage objective is the unweighted fraction of full-data high-gain
split thresholds whose adjacent bins are represented by at least one selected
or weighted surrogate point. It is binary threshold coverage, not gain-weighted
...
$(1 - 1/e)$ approximation guarantee~\cite{nemhauser1978submodular}. This defines
the HistDistill-Greedy selector: it achieves near-optimal coverage of the split
landscape, yet performs poorly by downstream predictive performance
...
trained. The pass costs $O(NFB)$ to build full-data histograms and $O(mFB)$ for
the surrogate, with shared bins and $O(FB)$ landscape memory per node.
