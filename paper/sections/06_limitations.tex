...
predict. Each boundary emerged from the diagnostic analyses rather than from an
assumed limitation.
...
Regression tasks require structure fidelity that small condensed sets did not
deliver at the studied budgets (\Cref{fig:regression}).

\begin{figure}[!t]
\centering
\includegraphics[width=0.72\columnwidth]{figures/phase2_e21_regression_decomposition_stacked.png}
\caption{%
...
speedup is $1.6\times$, far too small to compensate. Condensed HPO can sometimes
find a competitive configuration, but it often finds a different one rather than
...
\textbf{No-candidate-retraining selection of the single best method is unreliable.}
...
1.3 percentage points macro-AUROC, a positive but thin extension of the binary
results. Taken
