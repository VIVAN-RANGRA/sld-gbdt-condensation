...
metric. Extending distillation to tabular data has attracted recent
...
coverage distance; the CRAIG coreset method~\cite{mirzasoleiman2020craig} and
Gradient Matching (GradMatch)~\cite{killamsetty2021gradmatch} match gradient
norms via submodular maximisation; Error L2 Norm (EL2N) and Gradient Normed
(GraNd)~\cite{paul2021el2n} use early-training gradient norms. All of these
methods were designed for and evaluated on differentiable
...
landscape fidelity, measured by SLD, tracks their tabular AUROC. The
GBDT-native subsampling methods Gradient-based One-Side Sampling
(GOSS)~\cite{ke2017lightgbm} and Minimal Variance Sampling
...
The most closely related recent work is C\textsuperscript{2}TC
(Class-Adaptive Clustering for Tabular Condensation)~\cite{xu2026c2tc}
(ICDE 2026), a tabular condensation method that optimises class
