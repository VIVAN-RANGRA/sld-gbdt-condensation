...
each candidate. We do not benchmark C\textsuperscript{2}TC as a baseline in the
current grid because the experiment package was frozen around methods for which
we had compatible, reproducible implementations. The artifact repository exposes
the same condenser interface for adding C\textsuperscript{2}TC-generated sets;
SLD can then serve as an evaluation layer or as a selection criterion among its
class-allocation configurations.
