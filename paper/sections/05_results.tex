...
\subsection{Does a Training-Free Score Predict Downstream Performance?}
\label{sec:results-ordering}

A single split-regret score, computed without retraining any candidate, orders
condensation methods by their eventual downstream AUROC. The reason is
...
the achievable AUROC at once, a textbook Simpson's paradox (\Cref{fig:sldlaw}).
...
A selector that provably maximises split-threshold coverage is among the worst
condensers for downstream accuracy. HD-Greedy is built to cover as many
...
accurate methods occupy a separate region of the coverage--accuracy plane
...
accuracy lost because the condensed set induces the wrong tree structure, and a
leaf-estimate component, the accuracy lost to mis-estimated leaf values once the
...
weighting. The selection rule that follows is unambiguous: drive down landscape
discrepancy, not coverage.
...
splits, which matter most for accuracy, carry the widest margins and so flip
...
that corrupts those high-gain splits, as HD-Greedy does, pays a
disproportionate accuracy penalty.
...
\subsection{What Does the Most Faithful Method Buy?}
...
selector's accuracy while attaining the lowest landscape discrepancy among the
...
accuracy but lower cost. Condensed retraining runs several times faster than
...
datasets, but the three-dataset paired comparison is underpowered, so the honest
reading is convergence among reasonable condensers rather than a genuine Random
win (\Cref{tab:large-auroc}). This is the regime boundary: the refined family
