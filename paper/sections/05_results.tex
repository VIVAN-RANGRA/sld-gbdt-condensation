...
\textbf{Method} & \textbf{AUROC} & \textbf{Rank} &
  \textbf{Raw SLD}$_\infty$ & \textbf{Speedup} \\
\midrule
...
Lower split-regret and higher AUROC are better.}
...
\textbf{Finding 2: Within a dataset, lower raw SLD predicts higher AUROC, but
pooling across datasets is confounded.}
\Cref{fig:sldlaw} shows the pattern directly. Within each dataset, lower
raw $\delta_\infty$ corresponds to better retrained AUROC; after pooling
datasets, the trend can invert because dataset scale affects both raw SLD
magnitude and absolute AUROC. Raw SLD should therefore be used for
within-dataset, within-budget method comparison, not as a cross-dataset
absolute score.
...
method ranking.
This is the regime thesis in one comparison: HD-Refined beats Random in
24/26 small-to-mid-size datasets in the clustered audit (\Cref{tab:pairwise}),
yet Random has the largest point estimate on all three very large datasets
